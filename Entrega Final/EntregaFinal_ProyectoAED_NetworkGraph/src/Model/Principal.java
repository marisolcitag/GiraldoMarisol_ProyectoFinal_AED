...
public class Principal {
	...
	public ArrayList<Integer> transformData() {
		...
		if(networkMatrix!=null) {
			if(isMatrix) {
				Integer[][] matrix = networkMatrix.getNetwork();
	...
	public String BFSAdjacencyMatrix(NetworkDevice sourceNetwork) {
		...
		while(verteces.peek()!=null) {
			...
			for (int i = 0; i < length; i++) {
				...
				if(!visitedVerteces.contains(child) && networkMatrix.getEdge(position, i)!=null && position!=i) {
			...
			if(nodes!=null) {
				...
				while(iterator.hasNext()) {
					...
					if(!visitedVerteces.contains(edge.getVertexTwo())) {
						...
						if(networkDevice.equals(sourceNetwork)) {
	...
	public String DFSAdjacencyMatrix(NetworkDevice sourceNetwork, String response, HashSet<NetworkDevice> visitedVerteces, int level) {
		response += sourceNetwork.getId()+" ";
		visitedVerteces.add(sourceNetwork);
		int position = sourceNetwork.getId();
		System.out.println("ID: "+sourceNetwork.getId()+" IP: "+sourceNetwork.getIPAddress()+" TIPO:"+sourceNetwork.getType());
		boolean hasChilds = false;
		for (int i = 0; i < (int)networkMatrix.getNetwork()[0].length && !hasChilds; i++) {
		...
		if(!hasChilds) {
			...
			for (int i = 0; i < networkMatrix.getNetwork()[0].length; i++) {
				NetworkDevice child = networkMatrix.getNode(i);
				if(level==1) {
					System.out.println("Subred No"+(i+1));
				}
				if(!visitedVerteces.contains(child) && networkMatrix.getEdge(position, i)!=null) {
					DFSAdjacencyMatrix(child, response, visitedVerteces,level+1);
				}
			}
		}
		return response;
	...
	public String DFSAdjacencyList(NetworkDevice sourceNetwork, String response, HashSet<NetworkDevice> visitedVerteces, int level) {
		...
		if(nodes==null) {
			...
			while(iterator.hasNext()) {
				if(level==1) {
					System.out.println("Subred No"+(position+1));
				}
				IEdge<NetworkDevice, Integer> edge = iterator.next();
				if(!visitedVerteces.contains(edge.getVertexTwo())) {
					DFSAdjacencyList(edge.getVertexTwo(), response, visitedVerteces,level+1);					
				}
				position++;
			}
		}
	...
	public Object[] DijkstraAdjacencyMatrix(NetworkDevice sourceNode) {
		...
		int position = sourceNode.getId();
 		...
 		paths[position].append(position+"");
		for (int i = 0; i < visitedNodes.length; i++) {
			int value = LowestValueNotVisited(weights, visitedNodes);
			visitedNodes[value] = true;
			for (int j = 0; j < visitedNodes.length; j++) {
				int sum = 0;
				if(networkMatrix.getNetwork()[value][j]==null) {
					sum =  Integer.MAX_VALUE - 1000000; 
				}else {
					sum = networkMatrix.getNetwork()[value][j];
				}
				if(weights[j] > weights[value] + sum
						) {
					
					weights[j] = weights[value] + sum;
					if(paths[j].length()>0) {
						char aux = paths[j].charAt(paths[j].length()-1);
						paths[j].deleteCharAt(paths[j].length()-1);
						paths[j].append(value+" "+aux);
					}else {
						paths[j].append(paths[value].toString()+" "+j);
					}
				}
			}
		}
	...
	public Object[] DijkstraAdjacencyList(NetworkDevice sourceNode) {
		...
		int position = sourceNode.getId();
 		...
 		weights[position] = 0; 
 		Object[] nodes = networkList.getList().keySet().toArray();
		for (int i = 0; i < visitedNodes.length; i++) {
			int value = LowestValueNotVisited(weights, visitedNodes);
			visitedNodes[value] = true;
			for (int j = 0; j < visitedNodes.length; j++) {
