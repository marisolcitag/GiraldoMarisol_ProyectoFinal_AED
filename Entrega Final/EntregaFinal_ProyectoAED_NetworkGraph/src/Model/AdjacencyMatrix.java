...
public class AdjacencyMatrix<T,K extends Integer> implements IAdjacencyMatrix<T, K>{
	...
	public AdjacencyMatrix(int devicesAmount){
		network =  (K[][]) new Integer[devicesAmount][devicesAmount];
	...
	public void insertEdge(int firstNode, int secondNode, K value) {
		if(firstNode >= 0 && firstNode < devices.length && secondNode >=0 && secondNode < devices.length) {
			network[firstNode][secondNode] = (K) new Integer(value);
			network[secondNode][firstNode] = (K) new Integer(value);
		}
	}
